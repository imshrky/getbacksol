...
const STATIC_POSTS: string[] = [
  ...
  "Your referral link earns you 30% of the fee on every reclaim it brings in — for as long as it keeps happening, not just once. Find yours after connecting a wallet: getbacksol.com",
...
export async function getTelegramPost(dayIndex: number): Promise<string> {
  if (dayIndex % DYNAMIC_SLOT_EVERY === 0) return prizePoolPost();
